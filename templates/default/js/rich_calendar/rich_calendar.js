...
RichCalendar.attach_events = function(obj) {
	if (!window.IsTabletApplication()) {
		...
		RichCalendar.attach_event(obj, 'touchend', RichCalendar.onclick);	
	}
...
RichCalendar.detach_events = function(obj) {	
	if (!window.IsTabletApplication()) {
		...
		RichCalendar.detach_event(obj, 'touchend', RichCalendar.onclick);
	}
}
